...
Aycock and Horspool detail how a rightmost parse is evaluated from
their version of the Earley sets.
They suggest that in the case of multiple parses,
that their Earley sets can be iterated through,
pointing out where in their algorithm the decision points fall.
...
I saw that the ordering of certain lists in the Earley sets controlled
the order of the parses.
I realized that the order of those lists could be changed to anything useful.
For example, the order might be based on priorities assigned to the rules of the grammar.
