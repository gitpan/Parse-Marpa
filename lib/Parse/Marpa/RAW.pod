...
Each interface is allowed to use its own rules for symbol names.
...
For a somewhat introduction to these see the L<MDL document|Parse::Marpa::LANGUAGE>.

If C<min> and C<max> are both undefined, or both 1,
the rule is not a sequence production,
but is an ordinary BNF production for all purposes.
...
The C<start> option may be used to set the default for, or override
