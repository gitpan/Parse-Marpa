...
From one point of view, in fact, it is useless --
...
This document assumes the reader has a general knowledge of parsing,
...
It's important to not confuse Earley items with the LR(0).
...
parse initialization, token scanning, rule completion and rule prediction.
...
It has no successor.
The "effect" item is will be explained later.
...
any Earley item at earleme 2 which is looking for an C<e> is in luck.
...
just as we did for scanned rules.
...
In our example, whenever an Earley item is expecting to see an C<e>,
for example, it can also expect to the start of any rules that have
C<e> on their left hand side.
...
Marpa is smart about putting these things together in the same SDFA state
...
Second, as a result of the first,
the start and end earlemes will always be the same.
...
they don't need to have links or a lot the data the other items have.
...
No other Earley set will contain an Earley item for SDFA state 0.
...
The input parses successfully if, in the last Earleme, there is an Earley item
for a state containing a completed start rule.
For the special case of a successful null parse, SDFA state 0,
the state for the initial earley item,
would also contain a completed start rule.

Marpa always adds a special start rule,
so the state for the non-null complete start rule
and its state are not hard to spot.
It contains the rule with Marpa's new start symbol on the left hand side,
and the user's start symbol as the only symbol on the right hand side.
Here's the one in our example:

    S2: 8
    e['] ::= e .

A parse with this SDFA is successful at earleme I<N> if the Earley sets
contain an item for SDFA state 2, with a start earleme of 0 and an
end earleme of I<N>.
...
This means all Marpa's internal symbol end in a right square bracket.
...
Nullables have been a major issue with previous versions of Earley parsers.
...
but is not only allowed to appear in places
where the symbol cannot be nulled.
...
Marpa labels nulling symbols and
recognizes them directly, so it does not need to add empty rules when it
creates its own new nulling symbols.
However, where proper nullables had appeared on a rule's right hand side,
new rules often need to be added to account for all
possible combinations of the nulling and non-nulling variants.
...
That's exponential in the worst case,
and I don't like leaving exponential explosions in an algorithm, even unlikely ones.
(Could be it's that old Theory of Computation course,
still haunting me after all these years.)
In practice, I felt the generation of all possible combinations
...
Each piece is "factored out", so that every combination of nulling and non-nulling symbols is included.
...
and need to be "factored out" just as the original proper nullables must be.
Nonetheless this factoring can be done with 10 rules in CHAF,
while the original Aycock-Horspool factoring (NNF) would
require 16.
After three proper nullables, the advantage of CHAF becomes overwhelming.
With 4 proper nullables, there would be 13 rules for CHAF versus 32 for NNF.
With 5, 16 versus 64.

The user's semantics are preserved, because Marpa, while splitting and factoring the rules, inserts
codes to gathers and preserves the values of child nodes,
presenting them to the user's actions as if no
grammar rewriting had occurred.
...
Second, take 
the grammar used in the example and run it on the input text "2+2*3".
...
Get the C<show_status()> outputs from one of the ambiguous parses and reason out how
all of its Earley items came to be.

=head1 APPENDIX: DATA FOR THE EXAMPLE

Below are the MDL grammar and the full outputs of the example used in this
document.
The input text was "2+2".
