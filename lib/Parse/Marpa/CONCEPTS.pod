...
rule actions,
and a preamble.
Lex actions, rule actions and the preamble run
...
Semantics can be specified while the grammar is being built,
...
Null actions are different from the proper rule actions in another important way.
Null actions are run at or before parse creation time,
...
rule actions are compiled B<into closures>,
...
so a fixed value is usually what is wanted.
If you want to calculate a null value using a closure run at parse evaluation time,
the null action can return a reference to a closure.
...
Also, rule 2 has a proper rule action,
and it plays no role in the result either.

Here's an informally worded set of principles to explain Marpa's thinking in these matters:
...
If a grammar successfully parses the empty string, that's called a "null parse".
...
If you consider some of the rules and symbols that Marpa believes "don't count"
are important in your grammar.
Marpa null semantics probably can accommodate your ideas.
First, determine how your null semantics apply to every nullable symbol whenever it is
a "highest null symbol".
Then put those semantics into the symbol's null actions.
...
ignore the issue of parse volatility.
