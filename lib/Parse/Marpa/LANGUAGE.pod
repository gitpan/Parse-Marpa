...
Rule and lex actions are strings with Perl 5 code in them.
...
When a null symbol value is not set explicity set in an empty rule,
...
action sentence.
Before and after these may be other "non structural production sentences" -- sentences
...
In effect, a nullable symbol already is optional, and making it so
does the same thing twice
and causes Marpa to spin a lot of useless wheels internally.
...
you may wish to rewrite your sequence as BNF.
...
so that the first parse is the only parse.
...
The comment in the example before last notes that the code in that literal string could be replaced
using an internal routine.
Doing so would make C<self.marpa> simpler and
...
To write a lex action you'll need to know that stuff well.
...
name and returns the "canonical" MDL name.
The canonical name of an MDL symbol is the same as its raw interface name.
...
This cookie is needed to supply input to a parse with C<Parse::Marpa::Parse::earleme()>.
