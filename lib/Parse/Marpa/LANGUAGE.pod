...
It may not be all that hard to
write a high-level interface to Marpa which is
all of faster, easier to use, and more powerful.
...
and where context makes the difference clear,
...
For example, the following setting has
nulled symbols default to an at-sign.
...
By default, there is not lex prefix for a terminal.
(Or saying the same thing more pedantically,
the default is a lex prefix is
a regular expression which the empty string and only the empty string.)
...
You can deal with the semantics of sequence productions
...
Sequence productions can always be rewritten as BNF productions.
In fact, most parsers don't have sequence productions,
forcing you to write your sequences as BNF productions.
...
=item * The token length returned under MDL
in any grammar interface with a one-character-per-earleme model,
