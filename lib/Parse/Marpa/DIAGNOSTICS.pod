...
(in other words, Marpa uses the grammar to predict what terminals to look for next),
what the lexer is looking for can be a good clue to what the parse is doing.
...
you should do a quick deskcheck of your grammar.
Look at the output of
C<show_rules()> and C<show_symbols()> to see if there's anything not right.
...
C<trace_evaluation_choices> shows where in an ambiguous parse Marpa makes its choices,
and what choices it makes.
C<trace_iteration_changes> and C<trace_rules> traces the initialization of, and changes in, node values.

To do the real "into the Earley sets with gun and camera" stuff,
